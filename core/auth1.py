...
class TokenManager:
    ...
    _revoked: set[str] = set()   # jti revocation list (use Redis in production)
...
# ---------------------------------------------------------------------------
# In-memory user store (swap for DB-backed in production)
# ---------------------------------------------------------------------------

class UserStore:
    """Simple in-memory user store. Replace with SQLAlchemy repo for production."""
...
class AuthService:
    ...
    Inject a UserStore (or async DB repo) via constructor for testability.
    """
    ...
    def authenticate(self, username: str, password: str, totp_code: Optional[str] = None) -> Dict[str, Any]:
        ...
        if user.is_locked():
            audit.login_failure(username, ip="unknown", reason="account_locked")
        ...
        if not verify_password(password, user.password_hash):
            ...
            audit.login_failure(username, ip="unknown", reason="bad_password")
            raise AuthError("Invalid credentials")
        ...
        if settings.auth.mfa_enabled and user.mfa_secret:
            ...
            if not self._verify_totp(user.mfa_secret, totp_code):
                audit.login_failure(username, ip="unknown", reason="bad_mfa")
